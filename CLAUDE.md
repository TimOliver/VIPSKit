...
The Xcode project is generated by `Scripts/configure-project.rb` using the `xcodeproj` gem. Key configuration details:

- **VIPSKit target** (dynamic framework): Links vips.a via `-force_load $(BUILD_DIR)/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)/vips.a` plus system libraries `-lz -liconv -lresolv -lc++ -lexpat`
- **VIPSKitTests target**: Also needs `vips-static` as a package dependency for module resolution (the compiler must resolve `vips`/`CVIPS` modules recorded in VIPSKit.swiftmodule). This causes vips.a to be linked into the test bundle, so the test target also requires `-lz -liconv -lresolv -lc++` in OTHER_LDFLAGS
- **CVIPS.h** uses `Project` header visibility (not `Private`) to avoid Xcode generating a private module map warning
- **Test files** are added directly under the `Tests` group (no intermediate `VIPSKitTests` subgroup), matching the filesystem layout
- **COMPRESS_PNG_FILES = NO** on the test target prevents Xcode from converting PNGs to CgBI format that libpng/vips cannot read
...
### Swift 6.2 Static Library Naming

Swift 6.2 (Xcode 26.2) enforces that static libraries inside xcframeworks must use the `lib` prefix (e.g., `libvips.a` not `vips.a`). SPM will refuse to link a `.a` file without the prefix, causing "Undefined symbol" linker errors for all vips symbols. The vips-cocoa build scripts (`create-xcframework.sh`, `create-spm-xcframework.sh`) were updated to produce `libvips.a` for compatibility.
